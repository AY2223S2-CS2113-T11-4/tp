...
public class TaskList {
    ...
    public ArrayList<Task> getTaskWithPriority(String priority) {
        return (ArrayList<Task>) tasks.values().stream()
                .filter(t -> t.getPriority().toDisplayString() == priority)
                .collect(toList());
    }
    ...
    public HashSet<String> getAllPrioritiesInTaskList() {
        HashSet<String> priorities = new HashSet<>();
        tasks.values().forEach(task -> priorities.add(task.getPriority().toDisplayString()));
        return priorities;
    }
